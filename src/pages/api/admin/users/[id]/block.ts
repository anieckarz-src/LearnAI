...
export const POST: APIRoute = async ({ locals, params }) => {
  ...
  try {
    ...
    if (id === user.id) {
      return new Response(JSON.stringify({ success: false, error: "Cannot block yourself" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    const { data: targetUser } = await supabase.from("users").select("is_blocked").eq("id", id).single();
    ...
    return new Response(JSON.stringify({ success: false, error: "Failed to toggle user block" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
